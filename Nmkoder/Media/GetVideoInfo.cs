...
namespace Nmkoder.Media
{
    class GetVideoInfo
    {
        ...
        public static async Task<string> GetFfmpegOutputAsync(string path, string argsIn, string argsOut, string lineFilter = "", bool noCache = false, PT processType = PT.Background)
        {
            ...
            process.StartInfo.EnvironmentVariables["PATH"] = OsUtils.GetPathVar(Paths.GetBinPath());
            process.StartInfo.Arguments = Shell.BuildArguments($"{Shell.ChangeDir(Paths.GetBinPath())} " +
                $"ffmpeg -hide_banner -y {argsIn} {path.GetConcStr()} -i {path.Wrap()} {argsOut}");

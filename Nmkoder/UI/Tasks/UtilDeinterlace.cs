    ...
    class UtilDeinterlace
    {
        ...
        public static async Task Run()
        {
            ...
            try
            {
                ...
                // Whatever the Quick Convert tab is set to, so the mode and the QTGMC preset are
                // picked in one place rather than two. This utility is that tab's deinterlacer with
                // the encode settings decided for it.
                DeinterlaceRequest request = DeinterlaceUi.GetQuickConvertRequest();
        ...
        private static string DescribeNothingToDo(MediaFile file, DeinterlaceRequest request)
        {
            if (request.Mode == DeinterlaceMode.Disabled)
                return "Deinterlacing is switched off on the Quick Convert tab, and this utility runs whatever is set there.\n\n" +
                    "Set Deinterlace to Automatic or QTGMC over there, then run this again.";
            ...
            return $"'{file.Name}' is {scan}, so there is nothing to deinterlace and this would only copy it.\n\n" +
                "If you know better than the check does, set Deinterlace to QTGMC on the Quick Convert tab - " +
                "that runs it whatever the source says about itself.";
        }
    }
}

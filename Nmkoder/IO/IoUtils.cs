...
namespace Nmkoder.IO
{
    class IoUtils
    {
		...
		/// <summary> Loads an image, falling back to Magick.NET for formats Avalonia can't decode. </summary>
		public static Bitmap GetImage(string path, bool log = true)
		{
			try
			{
			...
			catch
			{
				try
				{
					...
					return MagickExtensions.LoadWithMagick(path);
				}
				catch (Exception e)
				{
					if (log)
		...
		public static async Task<Bitmap> GetThumbnail(string path)
		{
		...
		public static async Task<Size> GetVideoOrFramesRes (string path)
        {
            ...
            try
            {
				...
				{
					Bitmap thumb = await GetThumbnail(path);
					res = new Size(thumb.PixelSize.Width, thumb.PixelSize.Height);
				}
			}
			catch (Exception e)
            {
		...
		public static bool CanWriteToDir (string dir, ErrorMode errMode)
        {
			...
			catch (Exception e)
            {
				...
				if (errMode == ErrorMode.Messagebox)
					UiUtils.ShowMessageBoxAsync($"Can't write to {dir}!\n\n{e.Message}", UiUtils.MessageType.Error);
		...
		public static string GetHash (string path, Hash hashType, bool log = true)
		{
            ...
            try
            {
				...
				if (hashType == Hash.CRC32)
				{
					...
					crc.Append(stream);
					hashStr = BitConverter.ToUInt32(crc.GetCurrentHash(), 0).ToString();
				}
		...
		public static bool CheckImageValid (string path)
        {
            try
            {
				using Bitmap img = GetImage(path, false);
                return img != null && img.PixelSize.Width > 0 && img.PixelSize.Height > 0;
            }
			catch
            {
				return false;
		...
		public static long GetDiskSpace(string path, bool mbytes = true)
		{
			try
			{
				// Pick the mount point with the longest matching prefix - works for both
				// Windows drive letters and Unix mount points.
				DriveInfo drive = DriveInfo.GetDrives()
					.Where(d => d.IsReady && path.StartsWith(d.Name, StringComparison.OrdinalIgnoreCase))
					.OrderByDescending(d => d.Name.Length)
					.FirstOrDefault();

				if (drive != null)
					return mbytes ? (long)(drive.AvailableFreeSpace / 1024f / 1000f) : drive.AvailableFreeSpace;
			}
			catch (Exception e)
			{
